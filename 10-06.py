...
x_test = np.array([[-43.4047,-42.5683,-42.3697,-43.4229,-42.7953,-40.122,-35.1281,-44.5044,-29.601]], dtype=np.float32)
y_test = np.array([[23.2179,8.2306,7.0984]], dtype=np.float32)
...
W1 = tf.get_variable("W1", shape=[9, 50],
...
W4 = tf.get_variable("W4", shape=[50, 3],
     initializer=tf.contrib.layers.xavier_initializer())
...
with tf.Session() as sess:
    ...
    for epoch in range(num_epochs):
        ...
        ARR1= np.array(sess.run([hypothesis], feed_dict={X: x_test, Y: y_test}))
        for iteration in range(num_iterations):
           
            _, cost_val = sess.run([train, cost], feed_dict={X: x_data, Y: y_data, keep_prob: 0.7})
            avg_cost += cost_val / num_iterations
        print(f"Epoch: {(epoch + 1):04d}, Cost: {avg_cost:.9f}")
        if (epoch>0) and (((epoch%10)==0) or (epoch==301)):
    ...
    z_arr.insert(0,sum(sum(y_test[2:3]))/5)
               
    
    print(f"\nHypothesis:\n{h} ")
    print("x:",x_arr,"\ny:",y_arr,"\nz:",z_arr)

print(module1.cell(ARR1[0,0,0],ARR1[0,0,1],ARR1[0,0,2]/5))
