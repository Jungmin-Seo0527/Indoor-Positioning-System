...
x_data_all = np.loadtxt('rssi_kal2.txt', delimiter=',', skiprows=1, dtype='float32')
y_data_all = np.loadtxt('XYZrssi.txt', delimiter=',', skiprows=1, dtype='float32')
...
W1 = tf.get_variable("W1", shape=[9, 800],  ##tf.get_variable(<name>, <shape>, <initializer>): 입력된 이름의 변수를 생성하거나 반환함
...
# parameters
save_file = './train_test_kal22.ckpt'
...
with tf.Session() as sess:
    ...
    for epoch in range(num_epochs):
        ...
        print("Epoch: {(epoch + 1):04d}, Cost: {avg_cost:.9f}")  # f삭제
        if epoch == 0:
